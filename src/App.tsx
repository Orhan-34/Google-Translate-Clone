...
function App() {
	...
	useEffect(() => {
		...
		translate({ fromLanguage, toLanguage, text: debouncedText })
			.then((translatedTextResult) => {
				if (translatedTextResult === null || translatedTextResult === undefined)
					return;
	...
	const handleSpeakerClick = ({ language }: { language: Language }) => {
		if (language === toLanguage) {
			const utteranceTo = new SpeechSynthesisUtterance(translatedText);
			utteranceTo.lang = VOICE_LANGUAGES[language];
			utteranceTo.rate = 0.8;
			speechSynthesis.speak(utteranceTo);
		}
		if (language === fromLanguage) {
			const utteranceFrom = new SpeechSynthesisUtterance(text);
			utteranceFrom.lang = VOICE_LANGUAGES[language];
			utteranceFrom.rate = 0.8;
			...
			<Row>
				<Col xs={12} md={5}>
					...
					<div className="textarea-icon-buttons">
						<TextArea type={SectionType.From} text={text} onChange={setText} language={fromLanguage}/>
						...
						<div className="textarea-icon-buttons-from">
							<OverlayTrigger
								...
								placement={"right"}
								overlay={
									<Tooltip id={"tooltip-right"}>Listen to the text</Tooltip>
								}
								rootClose={true}
								transition={false}
							>
								<Button
									...
									hidden={
										text === "" ||
										loading ||
										fromLanguage === AUTO_DETECT_LANGUAGE
									}
									onClick={() => {
